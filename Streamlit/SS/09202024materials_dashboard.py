...
#---------------------------------------------------------------------------    
    ...
    if rec_calculation_method == cal_methods[0]:
        ...
        if num_pv_years != st.session_state.num_pv_years:
            st.session_state.pv_capacity = np.zeros(num_pv_years)
        ...
        if pv_material_intensity == "Constant Material Intensity":
            ...
            if st.button("Add",key="cap1"):
                st.session_state.pv_capacity[i] = pv_capacity
            pv_mint_data = pd.DataFrame({
                "Year":range(1,num_pv_years+1),
                "PV production":st.session_state.pv_capacity,
                #"start_year": start_year,
                #"end_year": end_year,
            ...
            i = years_list.index(year)
            pv_capacity = st.number_input(f"Enter the annual installed PV capacity (Specific technology considered) for Year {i+1}", min_value=0, step=1, key=f"pv_capacity2_{i}")
