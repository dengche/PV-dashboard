...
if "num_pv_years" not in st.session_state:
    st.session_state.num_pv_years = 0
...
if page == "Project Description":
    ...
    selected_use_option = st.selectbox("Scenario*", use_options)
    ...
    if st.button("Save"):
        ...
        if project_starting_year <= project_final_year:
            ...
            st.session_state.scenario = selected_supply_option
...
#---------------------------------------------------------------------------
    ...
    if selected_supply_option == "Manually input operating mines":
        ...
        if num_mines != st.session_state.num_mines:
            st.session_state.mine_annual_production = np.zeros(num_mines)
            st.session_state.mine_annual_growth = np.zeros(num_mines)
            st.session_state.mine_operating_years = np.zeros(num_mines) 
            st.session_state.num_mines = num_mines
        ### updated\\ 6/14/2024
        if num_mines == 0:
            mine_data = []
        if num_mines > 0:
            mines_list = [f"Mine {i}" for i in range(1, num_mines + 1)]
            mine = st.selectbox("Select the mine for input",mines_list)            
            i = mines_list.index(mine)
            ...
            annual_production_growth = st.number_input(f"Enter the annual production growth at first year for Mine {i+1}", min_value=0.0, step=0.01, key=f"annual_production_growth_{i}")
            operating_years = st.number_input(f"Enter the remaining operating years for Mine {i+1}", min_value=0, step=1, key=f"operating_years_{i}")
            if st.button("Add",key="mine"):
                st.session_state.mine_annual_production[i] = annual_production
                st.session_state.mine_annual_growth[i] = annual_production_growth
                st.session_state.mine_operating_years[i] = operating_years
        
            mine_data = pd.DataFrame({
                "Name":mines_list,
                #"start_year": start_year,
                #"end_year": end_year,
                "Annual production": st.session_state.mine_annual_production,
                "Annual production growth": st.session_state.mine_annual_growth,
                "Operating years": st.session_state.mine_operating_years
            })
    ...
    if selected_supply_option == "Calculate based on current production and future growth":
        supply_option = 2
        global_reserves = st.number_input("Enter the global reserves (tonnes)",min_value = 0)
        global_production = st.number_input("Enter the current annual global production (tonnes)", min_value=0)
...
#---------------------------------------------------------------------------
    ...
    bp_global_reserves_option = st.selectbox("Enter the global reserves (tonnes)", bp_option, key="bp_global_reserves_option1")
    if bp_global_reserves_option == "Use current production and average growth":
        bp_reserve_option = 2
        bp_current_supply = st.number_input("Enter the current supply from by-production", min_value=0, step=1, key="bp_current_supply1")
        bp_supply_growth = st.number_input("Enter the supply growth rate from by-production (%)", min_value=0, step=1, key="bp_supply_growth1")

    elif bp_global_reserves_option == "Manually input":
        bp_reserve_option = 1
        num_bp_hosts = st.selectbox("Select the number of host metals (maximum 3)", [0,1,2,3],key="num_bp_hosts1")
        ...
        if num_bp_hosts > 0:
            ...
            for i in range(num_bp_hosts):
                ...
                bp_host_metal_name[i] = st.text_input(f"Enter the host metal {i+1} name",key=f"bp_host_metal_name{i}")
                bp_annual_production[i] = st.number_input(f"Enter the host metal {i+1} annual production (tonnes)")
                bp_global_reserves[i] = st.number_input(f"Enter the host metal {i+1} gloabl reserves (tonnes)")
                bp_annual_production_growth[i] = st.number_input(f"Enter the host metal {i+1} annual production growth rate (%)")
                bp_hitchhiker_content[i] = st.number_input(f"Enter the host metal {i+1} hitchhiker content (ppm)")
                bp_hitchhiker_recovery_efficiency[i] = st.number_input(f"Enter the host metal {i+1} hitchhiker recovery efficiency (%)")
        else:
            bp_host_metal_name = [0]*num_bp_hosts
            bp_annual_production = np.zeros(3)
            bp_global_reserves = np.zeros(3)
...
#---------------------------------------------------------------------------
    ...
    if newpv_selected_panel_lifetime_option == "Fixed Lifetime":
        ...
        newpv_panel_lifetime = st.number_input("Panel lifetime (years)", min_value=0, step=1, value=25,key = "newpv_lifetime_input1")
    ...
    newpv_recycling_efficiency = st.number_input("Enter the recycling efficiency (%):", min_value=0, step=1, value=0,key = "newpv_efficiency1")    
    newpv_recycling_collection_efficiency = st.number_input("Enter the collection efficiency (%):", min_value=0, step=1, value=0,key = "newpv_collection1")
    newpv_percentage_panels_recycled = st.number_input("Enter the percentage of panels recycled (%):", min_value=0, step=1, value=0,key = "newpv_percentage1")
...
#---------------------------------------------------------------------------    
    ...
    rec_calculation_method = st.selectbox("Select the recycled materials calculation method",cal_methods,key = "pv calculation method1")
    if rec_calculation_method == cal_methods[0]:
        rec_cal_method = 1
        pv_capacity = st.number_input("Enter the installed PV capacity (Specific technology considered)", min_value=0, step=1, value=0, key="pv_capacity1")
        pv_recycling = st.number_input("Enter the percentage of panels recycled (%)",min_value=0, step=1, value=100, key = "pv_recycling1")
        pv_collection_efficiency = st.number_input("Enter the collection efficiency (%)",min_value=0, step=1, value=90, key = "pv_collection_efficiency1")
        pv_recycling_efficiency = st.number_input("Enter the recycling efficiency (%)",min_value=0, step=1, value=90, key = "pv_recycling_efficiency1")
        
        panel_lifetime_options = ["Fixed Lifetime", "Weibull Model - Regular Loss Scenario", "Weibull Model - Early Loss Scenario"]
        pv_panel_lifetime_option =  st.selectbox("Select the panel lifetime assumption", panel_lifetime_options, key = "pv_lifetime_option1")
        if pv_panel_lifetime_option == panel_lifetime_options[0]:
            pv_lifetime_option = 1
            pv_panel_lifetime = st.number_input("Panel Lifetime (years)", min_value=0, step=1, value=25,key = "pv_lifetime_input1")
        ...
        pv_material_intensity = st.selectbox("Material intensity of existing solar panels", material_intensity, key="pv_material_intensity1")

        if pv_material_intensity == "Constant Material Intensity":
            pv_mint = 1
            pv_avg_mint = st.number_input("Please enter the average material intensity of existing solar panels", min_value=0, step=1, key="pv_avg_mint1")

        elif pv_material_intensity == "Variable Material Intensity":
            pv_mint = 2
            num_pv_years = st.number_input("Enter the number of years", min_value=1, step=1)
            if num_pv_years != st.session_state.num_pv_years:
                st.session_state.pv_annual_mint = np.zeros(num_pv_years)
                st.session_state.num_pv_years = num_pv_years
            ...
            mint = st.number_input(f"Enter the annual material intensity for Year {i+1}", min_value=0, step=1, key=f"annual_mint{i}")
            if st.button("Add",key="mint1"):
                st.session_state.pv_annual_mint[i] = mint
        
            pv_mint_data = pd.DataFrame({
                "Year":range(1,num_pv_years+1),
                #"start_year": start_year,
                #"end_year": end_year,
                "Annual Material Intensity": st.session_state.pv_annual_mint,
            })
    ...
    if rec_calculation_method == cal_methods[1]:
        rec_cal_method = 2
        num_rec_years = st.number_input("Enter the number of years", min_value=1, step=1)
        if num_rec_years != st.session_state.num_rec_years:
            st.session_state.pv_material_supply = np.zeros(num_rec_years)
            st.session_state.num_rec_years = num_rec_years

        years_list2 = [f"Year {i}" for i in range(1, num_rec_years + 1)]
        year2 = st.selectbox("Select the year for input",years_list2)            
        y = years_list2.index(year2)
        m_supply = st.number_input(f"Enter the annual material supply for Year {y+1}", min_value=0, step=1, key=f"annual_supply{y}")
        if st.button("Add",key="material supply1"):
            st.session_state.pv_material_supply[y] = m_supply
        
        pv_annual_supply_data = pd.DataFrame({
            "Year":range(1,num_rec_years+1),
            #"start_year": start_year,
            #"end_year": end_year,
...
#---------------------------------------------------------------------------    
    ...
    if st.button("Save"):       
        ...
        if rec_cal_method == 1:
            st.session_state.input["pv_capacity"] = pv_capacity
            ...
            if pv_mint == 1:
                ...
                st.session_state.input["pv_annual_mint"] = pv_mint_data["Annual Material Intensity"]
        elif rec_cal_method == 2:
            st.session_state.input["pv_annual_supply_data"] = pv_annual_supply_data
            st.session_state.input["pv_annual_supply"] = pv_annual_supply_data["Annual Supply"]
...
if page == "Material Demand":
    ...
    if selected_supply_option == "Custom User Input":
        ...
        pv_year = st.selectbox("Select the year for input",years_list, key="pv_production_year") 
    ...
    pv_market_share = st.number_input("Enter the market share of the PV technology of interest (%)", min_value=0, step=1, value=0)
    pv_material_intensity = st.number_input("Enter the material intensity (kg/GWp)", min_value = 0, step=1, value = 0)
    ...
    if st.button("Save",key="pv_demand1"):
        ...
        st.session_state.input["pv_market_share"] = pv_market_share
        st.session_state.input["pv_material_intensity"] = pv_material_intensity
    ...
    if selected_inputs == "Material Demand":
        if "save3" not in st.session_state:
            ...
            if st.session_state.input["pv_user_input"] == 1:
                ...
                pv_demand_values = [inputs[pv_demand_lists[i]] for i in range(len(pv_demand_lists))]
                pv_demand_inputs = ["PV technology market share","PV technology material intensity"]
                pv_demand_df1 = st.session_state.input["pv_production"]
                pv_demand_df2 = pd.DataFrame({
                    "Inputs":pv_demand_inputs,
                    "Values":pv_demand_values
                })
                st.write(pv_demand_df1.to_html(index=False,justify='left', escape=False), unsafe_allow_html=True)
                st.write(pv_demand_df2.to_html(index=False,justify='left', escape=False), unsafe_allow_html=True)
...
if page == "Calculate Results":
    ...
    st.write("Results")
    self.get_res = st.button("Calculate", use_container_width=True)

    if self.get_res and "input" in st.session_state:
        # load the pysd file
        model = pysd.load('PVMat.py')
        data = st.session_state["input"]
    # Add content for Calculate Results page
        start_year = data["project_start_year"]
        end_year = data["project_end_year"]
        for year in range(start_year, end_year + 1):
            ind = year-start_year
            output = model.run(params={"Time":year,
                                       "PV Future Production User Input":data["pv_user_input"],
                                       "Planned Annual Incremental Capacity":data["annual_incremental_capacity1"],
                                       "PV Deployment Initial Incremental": data["pv_initial_incremental1"],
                                       "Current Annual PV Deployment": data["pv_annual_deployment1"],
                                       "Planned Annual Incremental Capacity User Input":data["annual_incremental_capacity2"],
                                       ...
                                       "PV Technology Market Share":data["pv_market_share"],
                                       "PV Technology Material Intensity":data["pv_material_intensity"],
                                       ...
                                       "Direct Mining Supply Manual":data["mines_data"]["Annual production"][ind]
                                       ###
                                       ...
                                       "Supply Gap Delay":data["supply_gap_delay"],
