...
def run (self):
    ...
    dashboard_functions = {
        ...
        "Material Supply" : self.material_supply,
        "Material Demand" : self.material_demand,
        "Material Global Stocks" : self.global_stocks,
        "Price Mechanism" : self.price_mechanism,
        "Modelling Inputs Review" : self.inputs_review,
    ...
    options = list(dashboard_functions.keys())
    selected_option = st.sidebar.selectbox("", options)

    for option in options:
            if option == selected_option:
                st.sidebar.markdown(f"<span style='font-weight: bold; font-size: 15px;'>{option}</span>", unsafe_allow_html=True)
            else:
                st.sidebar.markdown(f"<span style='font-size: 15px;'>{option}</span>", unsafe_allow_html=True)

    dashboard_functions[selected_option]()
