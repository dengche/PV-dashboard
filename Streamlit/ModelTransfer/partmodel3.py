...
from pysd.py_backend.functions import not_implemented_function, sum, if_then_else
...
@component.add(
    ...
    comp_subtype="Normal",
    depends_on={
        "pv_technology_production_capacity": 2,
        "price_threshold": 1,
...
def condition():
    return if_then_else(
        pv_technology_production_capacity() > price_threshold(),
        lambda: 1,
        lambda: if_then_else(
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"condition": 1, "counter_result": 1},
)
def annual_capacity_incremental():
    return np.maximum(0 + condition() - counter_result(), 0)
...
_integ_annual_pv_deployment = NonNegativeInteg(
