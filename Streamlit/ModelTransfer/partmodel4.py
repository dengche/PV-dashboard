...
import numpy as np
import xarray as xr
import pysd.py_backend.functions

from pysd.py_backend.functions import sum, if_then_else, not_implemented_function
...
def delayed_yield_effect():
    return DelayFixed( effect_on_material_byproduction_yield(), price_yield_delay(), 1
    )
...
def recycling_annual_pv():
    value = xr.DataArray(np.nan, {"PVDev": _subscript_dict["PVDev"]}, ["PVDev"])
    value.loc[["Y2023"]] = DelayFixed( actual_annual_material_demand(), 0, 0
    )
    value.loc[["Y2024"]] = DelayFixed( actual_annual_material_demand(), 1, 0
    )
    value.loc[["Y2025"]] = DelayFixed( actual_annual_material_demand(), 2, 0
    )
    value.loc[["Y2026"]] = DelayFixed( actual_annual_material_demand(), 3, 0
    )
    value.loc[["Y2027"]] = DelayFixed( actual_annual_material_demand(), 4, 0
    )
    value.loc[["Y2028"]] = DelayFixed( actual_annual_material_demand(), 5, 0
    )
    value.loc[["Y2029"]] = DelayFixed( actual_annual_material_demand(), 6, 0
    )
    value.loc[["Y2030"]] = DelayFixed( actual_annual_material_demand(), 7, 0
    )
    value.loc[["Y2031"]] = DelayFixed( actual_annual_material_demand(), 8, 0
    )
    value.loc[["Y2032"]] = DelayFixed( actual_annual_material_demand(), 9, 0
    )
    value.loc[["Y2033"]] = DelayFixed( actual_annual_material_demand(), 10, 0
    )
    value.loc[["Y2034"]] = DelayFixed( actual_annual_material_demand(), 11, 0
    )
    value.loc[["Y2035"]] = DelayFixed( actual_annual_material_demand(), 12, 0
    )
    value.loc[["Y2036"]] = DelayFixed( actual_annual_material_demand(), 13, 0
    )
    value.loc[["Y2037"]] = DelayFixed( actual_annual_material_demand(), 14, 0
    )
    value.loc[["Y2038"]] = DelayFixed( actual_annual_material_demand(), 15, 0
    )
    value.loc[["Y2039"]] = DelayFixed( actual_annual_material_demand(), 16, 0
    )
    value.loc[["Y2040"]] = DelayFixed( actual_annual_material_demand(), 17, 0
    )
    value.loc[["Y2041"]] = DelayFixed( actual_annual_material_demand(), 18, 0
    )
    value.loc[["Y2042"]] = DelayFixed( actual_annual_material_demand(), 19, 0
    )
    value.loc[["Y2043"]] = DelayFixed( actual_annual_material_demand(), 20, 0
    )
    value.loc[["Y2044"]] = DelayFixed( actual_annual_material_demand(), 21, 0
    )
    value.loc[["Y2045"]] = DelayFixed( actual_annual_material_demand(), 22, 0
    )
    value.loc[["Y2046"]] = DelayFixed( actual_annual_material_demand(), 23, 0
    )
    value.loc[["Y2047"]] = DelayFixed( actual_annual_material_demand(), 24, 0
    )
    value.loc[["Y2048"]] = DelayFixed( actual_annual_material_demand(), 25, 0
    )
    value.loc[["Y2049"]] = DelayFixed( actual_annual_material_demand(), 26, 0
    )
    value.loc[["Y2050"]] = DelayFixed( actual_annual_material_demand(), 27, 0
    )
    return value
...
def direct_mining_supply_rough_estimate():
    ...
    2: Calculate based on current production and future growth
    """
    return DelayFixed( annual_production(), 0, direct_mining_current_production_user_input()
    )
...
def delayed_dmgr_effect():
    return DelayFixed( effect_on_material_direct_mining_gr(), price_dmgr_delay(), 1
    )
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"direct_mining_affected": 1, "new_dmgr_factor": 1, "current_dm_gr": 2},
)
...
def delayed_yield_avg_effect():
    return DelayFixed( effect_on_material_byproduction_yield_avg(), price_yield_avg_delay(), 1
    )
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"counter_result": 1, "condition": 1},
)
def annual_capacity_incremental():
    return np.maximum(counter_result() + condition(), 0)
        ...
        np.maximum(
            if_then_else(
                ...
                lambda: if_then_else(
                    ...
                    lambda: DelayFixed( recycling_annual_pv(), 25, 0
                    ),
                ),
            ),
            0,
...
def yield_dummy_out():
    return np.maximum(DelayFixed( yield_dummy_in(), 1, 1), 0)
...
def dmgr_dummy_out():
    return np.maximum(DelayFixed(dmgr_dummy_in(), 1, 1), 0)
        ...
        np.maximum(
            if_then_else(
                ...
                lambda: if_then_else(
                    ...
                    lambda: DelayFixed( recycling_annual_expv(), 25, 0
                    ),
                ),
            ),
            0,
...
def yield_avg_dummy_out():
    return np.maximum(DelayFixed(yield_avg_dummy_in(), 1, 1), 0)
