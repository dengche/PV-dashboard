...
if "pv_annual_mint" not in st.session_state.input:
    st.session_state.input["pv_annual_mint"] = np.zeros(1)
if "pv_annual_mint1" not in st.session_state.input:
    st.session_state.input["pv_annual_mint1"] = np.zeros(1)
...
if "MS" not in st.session_state.output:
    st.session_state.output["MS"]= ""
if "MD" not in st.session_state.output:
...
if page == "Project Description":
    ...
    if selected_use_option == "I want to use an existing analysis":
        ...
        project_starting_year= st.number_input("Project timeline - starting year*",value=2030,disabled=True)
        project_final_year = st.number_input("Project timeline - final year*", min_value=2035, disabled=True)
...
#---------------------------------------------------------------------------
    ...
    if col2.button("**Note**",key="Direct Mining"):
        st.markdown(
                    ...
                    <br>**Variables**: Annual production, Year for production
...
#---------------------------------------------------------------------------    
    ...
    if rec_calculation_method == cal_methods[1]:
        ...
        m_supply = st.number_input(f"Enter the annual material supply for {year2}", min_value=0.0, step=0.1, key=f"annual_supply{y}")
...
#---------------------------------------------------------------------------    
    ...
    if st.button("Save"):       
        ...
        st.write(st.session_state.input["pv_capacity"])
...
if page == "Material Demand":
    ...
    if selected_supply_option == "Custom User Input":
        ...
        pv_material_intensity = st.number_input(f"Enter the material intensity (kg/GWp) for {years_list[ind]}", min_value = 0.0, step=0.1, value = 0.0,key=f"annual_pv_intensity2{years_list[ind]}")
    ...
    if False:
        ...
        if selected_market_share == "Constant":
            pv_tech_market_share = st.number_input("Enter the market share of the PV technology of interest (%)", min_value=0.0, step=0.1, value=0.0)
            st.session_state.input ["pv_tech_market_share"] = pv_tech_market_share
        else:
            current_pv_tech_market_share = st.number_input("Enter market share of the PV technology being assessed (%):", min_value=0.0, step=0.1, value=0.0)
            pv_tech_market_share_growth = st.number_input("Enter the future change in the market share (%):", min_value=0.0, step=0.1, value=0.0)
        ...
        if selected_material_intensity == "Preset Material Intensity Value":
            material_intensity = st.number_input("Enter the material intensity (kg/GWp)", min_value = 0.0, step=0.1, value = 0.0)
        ...
        if selected_mat_int_change == "I want to use a constant annual change (%)":
            delta_mat_int = st.number_input ("Enter the annual material intensity change (%):", min_value = 0.0, step=0.1, value = 0.0)
            ...
            for i in range(custom_years):
                ...
                mat_int_annual = st.number_input(f"Enter the material intensity (kg/GWp) for Year {year}", min_value=0.0, step=0.1, key=f"mat_int_annual_{i}")
    ...
    if selected_inputs == "Material Demand":
        if "save3" not in st.session_state:
            ...
            if st.session_state.input["pv_user_input"] == 1:
                ...
                pv_demand_inputs = ["Planned annual incremental capacity1","PV deployment initial incremental1","Current annual PV deployment1","PV technology market share","PV technology material intensity"]
...
if page == "Calculate Results":
    ...
    # Add content for Calculate Results page
        ...
        for year in range(start_year, end_year + 1):
            ...
            output = model.run(params={"Time":year,
                                       ...
                                       "Planned Annual Incremental Capacity User Input":convert_xr(start_year,data["annual_incremental_capacity2"]),
                                       ...
                                       "ExPV Installed":convert_xr(start_year,data["pv_capacity"]),
            ...
            columns_MS = ["Total Annual Recycling Supply from new PV",
                           "Total Annual Recycling Supply from existing PV",
                           "Total Annual Supply from byproduction",
                           "Total Annual Supply from Direct Mining"]
            columns_MD = ["Planned PV Capacity",
                          "Actual PV Technology Production",
                          "Actual Annual Material Demand",
                          "Annual Non PV Demand"]
            columns_MP = ["Material in the market Cumulative",
    ...
    if st.session_state["Cal"] == 1:
        ...
        if button5:
            title = st.session_state.input["project_title"]
            output = st.session_state.output["output"]
            name = f'raw_data_{title}.csv'
            csv = output.to_csv(index=True)
            st.download_button(label="Download data as CSV", data=csv, file_name=name, mime='text/csv')
            st.success(f"DataFrame saved to {name}") 
